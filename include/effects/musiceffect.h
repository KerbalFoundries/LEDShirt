...
{
  private: 
    //friend void SpectrumAnalyzerEffect(SpectrumAnalyzerEffect &sae);
...
class ChannelBeatEffect : public BeatEffect
{
  ...
  public:
    ...
    virtual void OnBeat()
    {
      ...
      CRGB b = CHSV(random(0, 255), random(0, 255), random(0, 255));
      ...
      CRGB c = CHSV(random(0, 255), random(0, 255), random(0, 255));
...
class SimpleColorBeat : public BeatEffectBase, protected virtual LEDStripEffect
{
  ...
  public:
    ...
    SimpleColorBeat(const char * pszName, double low, double high, double minElapsed)
      : LEDStripEffect(pszName), BeatEffectBase(0.25, 1.75, 0.125)
    {
    }

};

class IanColorBeat : public BeatEffectBase, protected virtual LEDStripEffect
{
  protected:

    int _iLastInsulator = -1;

    //just draw stuff. No beat detection here.       
    virtual void Draw()
    {
      BeatEffectBase::Draw();
      CRGB c = CRGB::Blue * gVURatio * g_AppTime.DeltaTime() * 0.75;
      setPixels(0, NUM_LEDS, c, true);
      LEDStripEffect::fadeAllChannelsToBlackBy(min(255.0,500 * g_AppTime.DeltaTime()));
  ...
  public:
    ...
    IanColorBeat(const char * pszName, double low, double high, double minElapsed)
      : LEDStripEffect(pszName), BeatEffectBase(1.0, 1.25, 0.125)
    {
    }
