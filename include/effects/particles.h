...
{ 
  ...
  public:
    ...
    virtual void Render()
    {
        ...
        if (_bErase)
          _pGFX[0]->setPixelsSmooth(_start, _length, CRGB::Black, false);
        ...
        if (_gapSize == 0)
        {
          for (int i = _start; i < _start+_length; i+=_lightSize)
          {
            ...
            _pGFX[0]->setPixelsSmooth(i, _lightSize, ColorFromPalette(_palette, iColor, 255 - 255 * FadeoutAmount(), _blend), true);
          }
        }
        else
        {
          ...
          for (float i = _start-totalSize; i < _start+_length+_lightSize; i++)
          {
              ...
              if (index == 0)
              {
                  ...
                  if (i + _startIndex > _start)
                    _pGFX[0]->setPixelsSmooth(i+_startIndex, _lightSize, c,true);
              }
          }
        }

        if (Age() < IgnitionTime() + PreignitionTime() && Age() >= PreignitionTime())
          _pGFX[0]->setPixelsSmooth(_start + random(0, _length), 1, CRGB::White, true);
    }
