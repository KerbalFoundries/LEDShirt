...
TaskHandle_t g_taskAccel  = nullptr;
...
// IE - Acceleration data object declared here to allow use elsewhere
DRAM_ATTR accelData g_accel;
...
{   
    ...
    xTaskCreatePinnedToCore(AccelLoopTaskEntry, "Accel Loop", STACK_SIZE, nullptr, ACCEL_PRIORITY, &g_taskAccel, ACCEL_CORE);
...
void loop()
{
    while(true)
    {
        ...
        #endif 
        #if ENABLE_TEMP_DEBUG
        EVERY_N_MILLIS(1000)
            {
                debugI("Voltage: %8.2f", M5.Axp.GetVBusVoltage());
            }
        #endif
        delay(10);        
    }
